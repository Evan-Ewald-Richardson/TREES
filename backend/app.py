"""GPX leaderboard FastAPI application."""
...
async def upload_gpx(gpxfile: UploadFile = File(...)):
    ...
    try:
        ...
        if not tracks:
            raise HTTPException(
                status_code=400, detail="No valid tracks or routes found in GPX file"
            )
...
def point_within_radius(
    p: Dict[str, Any], target: Dict[str, float], radius_m: int
) -> bool:
...
def create_course(body: Dict[str, Any], session: Session = Depends(get_session)):
    ...
    if not name or not isinstance(gates, list) or not gates:
        raise HTTPException(
            status_code=400, detail="Name and at least one gate are required"
        )
    ...
    for g in gates:
        ...
        if "checkpoints" in g:
            cps = g["checkpoints"]
            if not isinstance(cps, list) or any(
                not {"lat", "lon"} <= set(cp.keys()) for cp in cps
            ):
...
def get_leaderboard(
    course_id: int, session: Session = Depends(get_session), limit: int = 50
):
